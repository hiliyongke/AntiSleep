...
AntiSleep 是一款跨平台（macOS / Windows）桌面工具，通过系统级防休眠能力阻止屏幕自动锁屏，同时提供沉浸式氛围屏保体验。当前实现为 macOS `caffeinate` 与 Windows `SetThreadExecutionState`。
...
原因：这类能力会把产品从“防锁屏工具”拉向“系统级安全产品”，实现复杂度、平台差异和误锁风险都明显上升，但对当前核心场景价值有限。
...
pub fn start_prevention(mode: PreventionMode) -> Result<u32, String> {
    ...
    match mode {
        ...
        PreventionMode::System => {
            cmd.arg("-d").arg("-i").arg("-s");
        }
    }
}

#[cfg(target_os = "macos")]
pub fn stop_prevention(pid: u32) -> Result<(), String> {
...
1. Rust 防锁屏核心（macOS caffeinate + Windows SetThreadExecutionState）
...
| macOS caffeinate 兼容性问题 | 防锁屏失效 | 启动失败时显式报错，并保留平台实现替换空间 |
